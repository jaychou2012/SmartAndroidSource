 ...
 * }
 * 
 * ModifyFirstLetterNamingPolicy policy = new ModifyFirstLetterNamingPolicy(
 * 		LetterModifier.UPPER);
...
final class ModifyFirstLetterNamingPolicy extends RecursiveFieldNamingPolicy {
	...
	protected String translateName(String target, Type fieldType,
		...
		if (capitalizeFirstLetter && !Character.isUpperCase(firstCharacter)) {
			String modifiedTarget = modifyString(
					Character.toUpperCase(firstCharacter), target, ++index);
			return fieldNameBuilder.append(modifiedTarget).toString();
		} else if (!capitalizeFirstLetter
				&& Character.isUpperCase(firstCharacter)) {
			String modifiedTarget = modifyString(
					Character.toLowerCase(firstCharacter), target, ++index);
			return fieldNameBuilder.append(modifiedTarget).toString();
		} else {
			return target;
		}
	...
	private String modifyString(char firstCharacter, String srcString,
