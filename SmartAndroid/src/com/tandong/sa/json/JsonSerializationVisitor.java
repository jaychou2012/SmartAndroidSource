...
final class JsonSerializationVisitor implements ObjectNavigator.Visitor {
	...
	JsonSerializationVisitor(ObjectNavigator objectNavigator,
	...
	public void visitObjectField(FieldAttributes f, Type typeOfF, Object obj) {
		try {
			if (isFieldNull(f, obj)) {
				...
				// we should not get more specific component type yet since it
				// is
	...
	public void visitPrimitive(Object obj) {
		JsonElement json = obj == null ? JsonNull.INSTANCE : new JsonPrimitive(
				obj);
		assignToRoot(json);
	}

	private void addAsChildOfObject(FieldAttributes f,
			ObjectTypePair fieldValuePair) {
	...
	private void addChildAsElement(FieldAttributes f, JsonElement childElement) {
		root.getAsJsonObject().add(fieldNamingPolicy.translateName(f),
				childElement);
	}
	...
	private JsonElement getJsonElementForChild(ObjectTypePair fieldValueTypePair) {
		JsonSerializationVisitor childVisitor = new JsonSerializationVisitor(
				objectNavigator, fieldNamingPolicy, serializeNulls,
				serializers, context, ancestors);
	...
	private JsonElement findAndInvokeCustomSerializer(ObjectTypePair objTypePair) {
		Pair<JsonSerializer<?>, ObjectTypePair> pair = objTypePair
				.getMatchingHandler(serializers);
		if (pair == null) {
			return null;
		}
		...
		start(objTypePair);
		try {
			JsonElement element = serializer.serialize(objTypePair.getObject(),
					objTypePair.getType(), context);
	...
	public boolean visitFieldUsingCustomHandler(FieldAttributes f,
			Type declaredTypeOfField, Object parent) {
		try {
			...
			ObjectTypePair objTypePair = new ObjectTypePair(obj,
					declaredTypeOfField, false, false);
