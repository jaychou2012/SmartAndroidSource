 ...
 * with this class will override the default object creation that is defined by
 * the ObjectConstructor that this class is wrapping. Using this class with the
 * JSON framework provides the application with "pluggable" modules to customize
 * framework to suit the application's needs.
 * 
 * @author Joel Leitch
 */
final class MappedObjectConstructor implements ObjectConstructor {
	private static final UnsafeAllocator unsafeAllocator = UnsafeAllocator
			.create();
	private static final DefaultConstructorAllocator defaultConstructorAllocator = new DefaultConstructorAllocator(
			500);
	...
	public <T> T construct(Type typeOfT) {
		InstanceCreator<T> creator = (InstanceCreator<T>) instanceCreatorMap
				.getHandlerFor(typeOfT, false);
		if (creator != null) {
	...
	private <T> T constructWithAllocators(Type typeOfT) {
		try {
			...
			return (obj == null) ? unsafeAllocator.newInstance(clazz) : obj;
		} catch (Exception e) {
			throw new RuntimeException(
					("Unable to invoke no-args constructor for " + typeOfT
