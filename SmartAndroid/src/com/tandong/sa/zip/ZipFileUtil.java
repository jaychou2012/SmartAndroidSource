...
/**
 * Mainly methods to lookup Zip* class constructors. This is needed because Java
 * 6 doesn't have constructors with Charsets that were introduced in Java 7.
 */
class ZipFileUtil {
	 ...
	 */
	static ZipInputStream createZipInputStream(InputStream inStream,
			Charset charset) {
		if (charset == null)
		...
		try {
			Constructor<ZipInputStream> constructor = ZipInputStream.class
					.getConstructor(new Class[] { InputStream.class,
							Charset.class });
			return (ZipInputStream) constructor.newInstance(new Object[] {
					inStream, charset });
	...
	static ZipOutputStream createZipOutputStream(
			BufferedOutputStream outStream, Charset charset) {
		if (charset == null)
			return new ZipOutputStream(outStream);

		try {
			Constructor<ZipOutputStream> constructor = ZipOutputStream.class
					.getConstructor(new Class[] { OutputStream.class,
							Charset.class });
			return (ZipOutputStream) constructor.newInstance(new Object[] {
					outStream, charset });
		} catch (NoSuchMethodException e) {
			throw new IllegalStateException(MISSING_METHOD_PLEASE_UPGRADE, e);
		} catch (InstantiationException e) {
	...
	static ZipFile getZipFile(File src, Charset charset) throws IOException {
		...
		try {
			...
			return (ZipFile) constructor.newInstance(new Object[] { src,
					charset });
		} catch (NoSuchMethodException e) {
			throw new IllegalStateException(MISSING_METHOD_PLEASE_UPGRADE, e);
		} catch (InstantiationException e) {
