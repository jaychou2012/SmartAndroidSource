...
package com.tandong.sa.cropper;
...
public enum Edge {
	 ...
	 * @param imageSnapRadius
	 *            the radius (in pixels) at which the edge should snap to the
	 *            image
	 */
	public void adjustCoordinate(float x, float y, Rect imageRect,
			float imageSnapRadius, float aspectRatio) {
	...
	public void adjustCoordinate(float aspectRatio) {
		...
		switch (this) {
		case LEFT:
			mCoordinate = AspectRatioUtil.calculateLeft(top, right, bottom,
					aspectRatio);
			break;
		case TOP:
			mCoordinate = AspectRatioUtil.calculateTop(left, right, bottom,
					aspectRatio);
			break;
		case RIGHT:
			mCoordinate = AspectRatioUtil.calculateRight(left, top, bottom,
					aspectRatio);
			break;
		case BOTTOM:
			mCoordinate = AspectRatioUtil.calculateBottom(left, top, right,
					aspectRatio);
			break;
		}
	}
	 ...
	 * 
	 * @param Edge
	 *            the Edge that is about to be expanded
	 * @param imageRect
	 *            the rectangle of the picture
	 * @param aspectratio
	 *            the desired aspectRatio of the picture.
	 * 
	 * @return whether or not the new image would be out of bounds.
	 */
	public boolean isNewRectangleOutOfBounds(Edge edge, Rect imageRect,
			float aspectRatio) {
		...
		case LEFT:
			if (edge.equals(Edge.TOP)) {
				...
				float left = AspectRatioUtil.calculateLeft(top, right, bottom,
						aspectRatio);
		...
		case TOP:
			if (edge.equals(Edge.LEFT)) {
				...
				float top = AspectRatioUtil.calculateTop(left, right, bottom,
						aspectRatio);
		...
		case RIGHT:
			if (edge.equals(Edge.TOP)) {
				...
				float right = AspectRatioUtil.calculateRight(left, top, bottom,
						aspectRatio);
		...
		case BOTTOM:
			if (edge.equals(Edge.LEFT)) {
				...
				float bottom = AspectRatioUtil.calculateBottom(left, top,
						right, aspectRatio);
	 ...
	 */
	private boolean isOutOfBounds(float top, float left, float bottom,
			float right, Rect imageRect) {
		return (top < imageRect.top || left < imageRect.left
				|| bottom > imageRect.bottom || right > imageRect.right);
	}
	 ...
	 * 
	 * @param imageRect
	 *            the bounding rectangle of the image to snap to
	 ...
	 * 
	 * @param x
	 *            the x-position that the left edge is dragged to
	 * @param imageRect
	 *            the bounding box of the image that is being cropped
	 * @param imageSnapRadius
	 *            the snap distance to the image edge (in pixels)
	 * @return the actual x-position of the left edge
	 */
	private static float adjustLeft(float x, Rect imageRect,
			float imageSnapRadius, float aspectRatio) {
		...
		if (x - imageRect.left < imageSnapRadius)
			...
			if (((Edge.RIGHT.getCoordinate() - x) / aspectRatio) <= MIN_CROP_LENGTH_PX)
				resultXVert = Edge.RIGHT.getCoordinate()
						- (MIN_CROP_LENGTH_PX * aspectRatio);
	 ...
	 * 
	 * @param x
	 *            the x-position that the right edge is dragged to
	 * @param imageRect
	 *            the bounding box of the image that is being cropped
	 * @param imageSnapRadius
	 *            the snap distance to the image edge (in pixels)
	 ...
	 */
	private static float adjustRight(float x, Rect imageRect,
			float imageSnapRadius, float aspectRatio) {
		...
		if (imageRect.right - x < imageSnapRadius)
			...
			if (((x - Edge.LEFT.getCoordinate()) / aspectRatio) <= MIN_CROP_LENGTH_PX) {
				resultXVert = Edge.LEFT.getCoordinate()
						+ (MIN_CROP_LENGTH_PX * aspectRatio);
			}
	 ...
	 * 
	 * @param y
	 *            the x-position that the top edge is dragged to
	 * @param imageRect
	 *            the bounding box of the image that is being cropped
	 * @param imageSnapRadius
	 *            the snap distance to the image edge (in pixels)
	 ...
	 */
	private static float adjustTop(float y, Rect imageRect,
			float imageSnapRadius, float aspectRatio) {
		...
		if (y - imageRect.top < imageSnapRadius)
			...
			if (((Edge.BOTTOM.getCoordinate() - y) * aspectRatio) <= MIN_CROP_LENGTH_PX)
				resultYVert = Edge.BOTTOM.getCoordinate()
						- (MIN_CROP_LENGTH_PX / aspectRatio);
	 ...
	 * 
	 * @param y
	 *            the x-position that the bottom edge is dragged to
	 * @param imageRect
	 *            the bounding box of the image that is being cropped
	 * @param imageSnapRadius
	 *            the snap distance to the image edge (in pixels)
	 ...
	 */
	private static float adjustBottom(float y, Rect imageRect,
			float imageSnapRadius, float aspectRatio) {
		...
		if (imageRect.bottom - y < imageSnapRadius)
			...
			if (((y - Edge.TOP.getCoordinate()) * aspectRatio) <= MIN_CROP_LENGTH_PX)
				resultYHoriz = Edge.TOP.getCoordinate()
						+ (MIN_CROP_LENGTH_PX / aspectRatio);
