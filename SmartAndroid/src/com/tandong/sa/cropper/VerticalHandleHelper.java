...
class VerticalHandleHelper extends HandleHelper {
	...
	@Override
	void updateCropWindow(float x, float y, float targetAspectRatio,
		...
		final float targetHeight = AspectRatioUtil.calculateHeight(left, right,
				targetAspectRatio);
		...
		if (Edge.TOP.isOutsideMargin(imageRect, snapRadius)
				&& !mEdge.isNewRectangleOutOfBounds(Edge.TOP, imageRect,
						targetAspectRatio)) {
		...
		if (Edge.BOTTOM.isOutsideMargin(imageRect, snapRadius)
				&& !mEdge.isNewRectangleOutOfBounds(Edge.BOTTOM, imageRect,
						targetAspectRatio)) {
