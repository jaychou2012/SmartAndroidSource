...
final class ObjectTypePair {
	...
	public String toString() {
		return String.format("preserveType: %b, type: %s, obj: %s",
				preserveType, type, obj);
	}
	...
	<HANDLER> Pair<HANDLER, ObjectTypePair> getMatchingHandler(
		...
		if (!preserveType && obj != null) {
			...
			if (handler != null) {
				return new Pair<HANDLER, ObjectTypePair>(handler,
						moreSpecificType);
			}
		}
		// Try the specified type
		handler = handlers.getHandlerFor(type, systemOnly);
		return handler == null ? null : new Pair<HANDLER, ObjectTypePair>(
				handler, this);
	}
	...
	// This takes care of situations where the field was declared as an Object,
	// but the
	// actual value contains something more specific. See Issue 54.
	// TODO (inder): This solution will not work if the field is of a generic
	// type, but
	// the actual object is of a raw type (which is a sub-class of the generic
	// type).
	...
	@Override
	public int hashCode() {
		// Not using type.hashCode() since I am not sure if the subclasses of
		// type reimplement
