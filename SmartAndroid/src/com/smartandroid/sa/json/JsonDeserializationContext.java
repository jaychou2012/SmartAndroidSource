...
public class JsonDeserializationContext {
	...
	private <T> T fromJsonArray(Type arrayType, JsonArray jsonArray,
			JsonDeserializationContext context, boolean systemOnly)
		...
		objectNavigator.accept(new ObjectTypePair(null, arrayType, true,
				systemOnly), visitor);
	...
	private <T> T fromJsonObject(Type typeOfT, JsonObject jsonObject,
		...
		JsonObjectDeserializationVisitor<T> visitor = new JsonObjectDeserializationVisitor<T>(
				...
				objectConstructor, deserializers, context);
		objectNavigator.accept(new ObjectTypePair(null, typeOfT, true,
				systemOnly), visitor);
		return visitor.getTarget();
	}
	...
	private <T> T fromJsonPrimitive(Type typeOfT, JsonPrimitive json,
			...
			throws JsonParseException {
		JsonObjectDeserializationVisitor<T> visitor = new JsonObjectDeserializationVisitor<T>(
				json, typeOfT, objectNavigator, fieldNamingPolicy,
				objectConstructor, deserializers, context);
		objectNavigator.accept(new ObjectTypePair(json.getAsObject(), typeOfT,
				true, systemOnly), visitor);
	...
	public <T> T deserialize(JsonElement json, Type typeOfT)
		...
		if (json == null || json.isJsonNull()) {
			...
			Object object = fromJsonObject(typeOfT, json.getAsJsonObject(),
					this, false);
			return (T) object;
		} else if (json.isJsonPrimitive()) {
			Object primitive = fromJsonPrimitive(typeOfT,
					json.getAsJsonPrimitive(), this, false);
			return (T) primitive;
		} else {
	...
	@SuppressWarnings("unchecked")
	public <T> T deserializeDefault(JsonElement json, Type typeOfT)
			throws JsonParseException {
		if (json == null || json.isJsonNull()) {
			return null;
		} else if (json.isJsonArray()) {
			...
			Object primitive = fromJsonPrimitive(typeOfT,
					json.getAsJsonPrimitive(), this, true);
			return (T) primitive;
		} else {
			throw new JsonParseException("Failed parsing JSON source: " + json
					+ " to Json");
