...
public class StaggeredGridView extends ViewGroup {
	...
	private void dumpItemPositions() {
		...
		for (int i = 0; i < mColCount; i++) {
			...
			if (!found) {
				Log.d(TAG, "!!! No top item found for column " + i + " value "
						+ mItemTops[i]);
			}
		}
		Log.d(TAG, " => Bottoms:");
		for (int i = 0; i < mColCount; i++) {
			...
			if (!found) {
				Log.d(TAG, "!!! No bottom item found for column " + i
						+ " value " + mItemBottoms[i]);
			}
		}
	}
	...
	public Parcelable onSaveInstanceState() {
		...
		if (getChildCount() > 0) {
			ss.topOffset = getChildAt(0).getTop() - mItemMargin
					- getPaddingTop();
		}
		return ss;
	}
