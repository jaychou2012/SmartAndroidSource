...
import com.smartandroid.sa.zUImageLoader.core.assist.LoadedFrom;
import com.smartandroid.sa.zUImageLoader.core.assist.ViewScaleType;
import com.smartandroid.sa.zUImageLoader.core.assist.FailReason.FailType;
import com.smartandroid.sa.zUImageLoader.core.decode.ImageDecoder;
import com.smartandroid.sa.zUImageLoader.core.decode.ImageDecodingInfo;
import com.smartandroid.sa.zUImageLoader.core.download.ImageDownloader;
import com.smartandroid.sa.zUImageLoader.core.download.ImageDownloader.Scheme;
import com.smartandroid.sa.zUImageLoader.core.imageaware.ImageAware;
import com.smartandroid.sa.zUImageLoader.core.listener.ImageLoadingListener;
import com.smartandroid.sa.zUImageLoader.core.listener.ImageLoadingProgressListener;
import com.smartandroid.sa.zUImageLoader.utils.IoUtils;
...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
	...
	private boolean delayIfNeed() {
		if (options.shouldDelayBeforeLoading()) {
			L.d(LOG_DELAY_BEFORE_LOADING, options.getDelayBeforeLoading(),
					memoryCacheKey);
			try {
		...
		try {
			...
			if (bitmap == null || bitmap.getWidth() <= 0
				...
				if (options.isCacheOnDisk() && tryCacheImageOnDisk()) {
					...
					if (imageFile != null) {
						imageUriForDecoding = Scheme.FILE.wrap(imageFile
								.getAbsolutePath());
					}
				}
	...
	private Bitmap decodeImage(String imageUri) throws IOException {
		...
		ImageDecodingInfo decodingInfo = new ImageDecodingInfo(memoryCacheKey,
				imageUri, uri, targetSize, viewScaleType, getDownloader(),
				options);
	...
	private boolean resizeAndSaveImage(int maxWidth, int maxHeight)
		...
		if (targetFile != null && targetFile.exists()) {
			...
			ImageDecodingInfo decodingInfo = new ImageDecodingInfo(
					memoryCacheKey, Scheme.FILE.wrap(targetFile
							.getAbsolutePath()), uri, targetImageSize,
	...
	private boolean fireProgressEvent(final int current, final int total) {
		...
		if (progressListener != null) {
			Runnable r = new Runnable() {
				@Override
				public void run() {
					progressListener.onProgressUpdate(uri,
							imageAware.getWrappedView(), current, total);
				}
			};
			...
			public void run() {
				if (options.shouldShowImageOnFail()) {
					imageAware.setImageDrawable(options
							.getImageOnFail(configuration.resources));
				}
				listener.onLoadingFailed(uri, imageAware.getWrappedView(),
