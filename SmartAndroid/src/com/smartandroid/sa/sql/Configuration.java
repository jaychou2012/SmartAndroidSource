package com.smartandroid.sa.sql;
...
import com.smartandroid.sa.sql.util.ReflectionUtils;
...
public class Configuration {
	...
	public static class Builder {
		...
		public Builder addTypeSerializer(
				Class<? extends TypeSerializer> typeSerializer) {
		...
		public Builder addTypeSerializers(
		...
		public Builder setTypeSerializers(
				Class<? extends TypeSerializer>... typeSerializers) {
		...
		public Configuration create() {
			...
			if (mModelClasses != null) {
				...
				final String modelList = ReflectionUtils.getMetaData(mContext,
						AA_MODELS);
				if (modelList != null) {
					configuration.mModelClasses = loadModelList(modelList
							.split(","));
				}
			}
			...
			if (mTypeSerializers != null) {
				...
				final String serializerList = ReflectionUtils.getMetaData(
						mContext, AA_SERIALIZERS);
				if (serializerList != null) {
					configuration.mTypeSerializers = loadSerializerList(serializerList
							.split(","));
				}
			}
		...
		private int getMetaDataDatabaseVersionOrDefault() {
			Integer aaVersion = ReflectionUtils.getMetaData(mContext,
					AA_DB_VERSION);
		...
		private String getMetaDataSqlParserOrDefault() {
			final String mode = ReflectionUtils.getMetaData(mContext,
					AA_SQL_PARSER);
			if (mode == null) {
		...
		private List<Class<? extends Model>> loadModelList(String[] models) {
			...
			final ClassLoader classLoader = mContext.getClass()
					.getClassLoader();
			for (String model : models) {
				try {
					Class modelClass = Class.forName(model.trim(), false,
							classLoader);
		...
		private List<Class<? extends TypeSerializer>> loadSerializerList(
				String[] serializers) {
			...
			for (String serializer : serializers) {
				try {
					Class serializerClass = Class.forName(serializer.trim(),
							false, classLoader);
