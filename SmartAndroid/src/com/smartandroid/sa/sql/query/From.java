package com.smartandroid.sa.sql.query;
...
import com.smartandroid.sa.sql.content.ContentProvider;
import com.smartandroid.sa.sql.query.Join.JoinType;
...
public final class From implements Sqlable {
	...
	public <T extends Model> List<T> execute() {
		if (mQueryBase instanceof Select) {
			...
			Cache.getContext().getContentResolver()
					.notifyChange(ContentProvider.createUri(mType, null), null);
			return null;
	...
	public <T extends Model> T executeSingle() {
		if (mQueryBase instanceof Select) {
			limit(1);
			return (T) SQLiteUtils.rawQuerySingle(mType, toSql(),
					getArguments());
	 ...
	 * 
	 * @return <code>true</code> if the query returns at least one row;
