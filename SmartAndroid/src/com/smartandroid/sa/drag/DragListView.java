...
public class DragListView extends ListView {
	...
	public DragListView(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			...
			mTrackDragSort = a.getBoolean(
					getResources().getIdentifier(
							"DragSortListView_track_drag_sort", "styleable",
			...
			mFloatAlpha = a.getFloat(
					getResources().getIdentifier(
							"DragSortListView_float_alpha", "styleable",
							context.getPackageName()), mFloatAlpha);
			// mFloatAlpha =
			// a.getFloat(R.styleable.DragSortListView_float_alpha,
			// mFloatAlpha);
			mCurrFloatAlpha = mFloatAlpha;

			mDragEnabled = a.getBoolean(
					getResources().getIdentifier(
							"DragSortListView_drag_enabled", "styleable",
							context.getPackageName()), mDragEnabled);
			// mDragEnabled = a.getBoolean(
			...
			mSlideRegionFrac = Math.max(0.0f, Math.min(1.0f, 1.0f - a.getFloat(
					getResources().getIdentifier(
							...
							"styleable", context.getPackageName()), 0.75f)));
			// mSlideRegionFrac = Math.max(0.0f, Math.min(1.0f, 1.0f -
			// a.getFloat(
			...
			float frac = a.getFloat(
					getResources().getIdentifier(
							"DragSortListView_drag_scroll_start", "styleable",
							context.getPackageName()), mDragUpScrollStartFrac);
			// float frac = a.getFloat(
			...
			if (useDefault) {
				boolean removeEnabled = a.getBoolean(
						getResources().getIdentifier(
								"DragSortListView_remove_enabled", "styleable",
								context.getPackageName()), false);
				// boolean removeEnabled = a.getBoolean(
				// R.styleable.DragSortListView_remove_enabled, false);
				int removeMode = a.getInt(
						getResources().getIdentifier(
								"DragSortListView_remove_mode", "styleable",
				...
				boolean sortEnabled = a.getBoolean(
						getResources().getIdentifier(
								"DragSortListView_sort_enabled", "styleable",
				...
				int dragHandleId = a.getResourceId(
						getResources().getIdentifier(
								"DragSortListView_drag_handle_id", "styleable",
	 ...
	 * {@link DragListView.DragScrollProfile}.
	 * 
	 * @param ssp
	 */
	 ...
	 * and time. Use
	 * {@link DragListView#setDragScrollProfile(DragScrollProfile)} to set
	 * custom profile.
	 * 
	 * @author heycosmo
	 * 
