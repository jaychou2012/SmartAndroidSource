 ...
 *******************************************************************************/
package com.smartandroid.sa.pullto;
...
import com.smartandroid.sa.pullto.internal.FlipLoadingLayout;
import com.smartandroid.sa.pullto.internal.LoadingLayout;
import com.smartandroid.sa.pullto.internal.RotateLoadingLayout;
import com.smartandroid.sa.pullto.internal.Utils;
import com.smartandroid.sa.pullto.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	private void init(Context context, AttributeSet attrs) {
		...
		TypedArray a = context.obtainStyledAttributes(
				attrs,
				new int[] { getResources().getIdentifier("PullToRefresh",
						"styleable", context.getPackageName()) });
		// TypedArray a = context.obtainStyledAttributes(attrs,
		// R.styleable.PullToRefresh);

		if (a.hasValue(getResources().getIdentifier("PullToRefresh_ptrMode",
				"styleable", context.getPackageName()))) {
			// if (a.hasValue(R.styleable.PullToRefresh_ptrMode)) {
			mMode = Mode.mapIntToValue(a.getInteger(
					getResources().getIdentifier("PullToRefresh_ptrMode",
							"styleable", context.getPackageName()), 0));
			// R.styleable.PullToRefresh_ptrMode, 0));
		}

		if (a.hasValue(getResources().getIdentifier(
				"PullToRefresh_ptrAnimationStyle", "styleable",
		 ...
		 */
		if (a.hasValue(getResources().getIdentifier(
				"PullToRefresh_ptrRefreshableViewBackground", "styleable",
				context.getPackageName()))) {
			// if
			// (a.hasValue(R.styleable.PullToRefresh_ptrRefreshableViewBackground))
			// {
		...
		} else if (a.hasValue(getResources().getIdentifier(
				"PullToRefresh_ptrAdapterViewBackground", "styleable",
				context.getPackageName()))) {
		...
		if (a.hasValue(getResources().getIdentifier(
				"PullToRefresh_ptrOverScroll", "styleable",
				context.getPackageName()))) {
			// if (a.hasValue(R.styleable.PullToRefresh_ptrOverScroll)) {
			mOverScrollEnabled = a.getBoolean(
					getResources().getIdentifier("PullToRefresh_ptrOverScroll",
		...
		if (a.hasValue(getResources().getIdentifier(
				...
				"styleable", context.getPackageName()))) {
			// if
			// (a.hasValue(R.styleable.PullToRefresh_ptrScrollingWhileRefreshingEnabled))
			// {
