...
import com.smartandroid.sa.sherlock.internal.nineoldandroids.widget.NineHorizontalScrollView;
...
public class ScrollingTabContainerView extends NineHorizontalScrollView
	...
	private IcsLinearLayout createTabLayout() {
		final TabsLinearLayout tabLayout = (TabsLinearLayout) LayoutInflater
	...
	private IcsSpinner createSpinner() {
		final IcsSpinner spinner = new IcsSpinner(getContext(), null,
				getResources().getIdentifier("actionDropDownStyle", "attr",
						c.getPackageName()));
	...
	protected void onConfigurationChanged(Configuration newConfig) {
		...
		TypedArray a = getContext().obtainStyledAttributes(
				null,
				new int[] { getResources().getIdentifier("SherlockActionBar",
						"styleable", c.getPackageName()) },
				getResources().getIdentifier("actionBarStyle", "attr",
						c.getPackageName()), 0);
		// R.styleable.SherlockActionBar, R.attr.actionBarStyle, 0);
		setContentHeight(a.getLayoutDimension(
				getResources().getIdentifier("SherlockActionBar_height",
						"styleable", c.getPackageName()), 0));
		// R.styleable.SherlockActionBar_height, 0));
		a.recycle();
	}
	...
	private TabView createTabView(ActionBar.Tab tab, boolean forAdapter) {
		...
		final TabView tabView = (TabView) mInflater.inflate(
				getResources().getIdentifier("abs__action_bar_tab", "layout",
						c.getPackageName()), null);
		...
		if (forAdapter) {
			...
			tabView.setLayoutParams(new ListView.LayoutParams(
					ListView.LayoutParams.MATCH_PARENT, mContentHeight));
		} else {
				...
				if (text != null) {
					if (mTextView == null) {
						CapitalizingTextView textView = new CapitalizingTextView(
								getContext(), null, getResources()
										.getIdentifier("actionBarTabTextStyle",
												"attr", ct.getPackageName()));
