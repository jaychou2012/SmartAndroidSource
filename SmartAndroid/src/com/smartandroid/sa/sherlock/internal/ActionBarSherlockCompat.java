...
import com.smartandroid.sa.BuildConfig;
import com.smartandroid.sa.sherlock.ActionBarSherlock;
import com.smartandroid.sa.sherlock.app.ActionBar;
import com.smartandroid.sa.sherlock.internal.app.ActionBarImpl;
import com.smartandroid.sa.sherlock.internal.view.StandaloneActionMode;
...
import com.smartandroid.sa.sherlock.internal.widget.ActionBarContainer;
import com.smartandroid.sa.sherlock.internal.widget.ActionBarContextView;
import com.smartandroid.sa.sherlock.internal.widget.ActionBarView;
import com.smartandroid.sa.sherlock.internal.widget.IcsProgressBar;
...
public class ActionBarSherlockCompat extends ActionBarSherlock implements
		MenuBuilder.Callback, com.smartandroid.sa.sherlock.view.Window.Callback,
		...
		if (mode != null) {
			...
			if (mActionModeView == null) {
				ViewStub stub = (ViewStub) mDecor
						.findViewById(getThemedContext().getResources()
								.getIdentifier("abs__action_mode_bar_stub",
										"id",
										getThemedContext().getPackageName()));
	...
	private IcsProgressBar getCircularProgressBar(boolean shouldInstallDecor) {
		...
		mCircularProgressBar = (IcsProgressBar) mDecor
				.findViewById(getThemedContext().getResources().getIdentifier(
						"abs__progress_circular", "id",
						getThemedContext().getPackageName()));
	...
	private void installDecor() {
		...
		if (mContentParent == null) {
			...
			wActionBar = (ActionBarView) mDecor.findViewById(getThemedContext()
					.getResources().getIdentifier("abs__action_bar", "id",
							getThemedContext().getPackageName()));
			...
			if (wActionBar != null) {
				...
				if (splitWhenNarrow) {
					splitActionBar = getResources_getBoolean(
							mActivity,
							getThemedContext().getResources().getIdentifier(
				...
				final ActionBarContainer splitView = (ActionBarContainer) mDecor
						.findViewById(getThemedContext().getResources()
								.getIdentifier("abs__split_action_bar", "id",
										getThemedContext().getPackageName()));
				...
				if (splitView != null) {
					...
					mActionModeView = (ActionBarContextView) mDecor
							.findViewById(getThemedContext()
									.getResources()
									.getIdentifier("abs__action_context_bar",
											"id",
											getThemedContext().getPackageName()));
	...
	private ViewGroup generateLayout() {
		...
		TypedArray a = mActivity.getTheme().obtainStyledAttributes(
				new int[] { getThemedContext().getResources().getIdentifier(
						"SherlockTheme", "styleable",
						getThemedContext().getPackageName()) });
		// R.styleable.SherlockTheme);

		if (!a.hasValue(getThemedContext().getResources().getIdentifier(
				"SherlockTheme_windowActionBar", "styleable",
				getThemedContext().getPackageName()))) {
		...
		if (a.getBoolean(
				getThemedContext().getResources().getIdentifier(
						"SherlockTheme_windowNoTitle", "styleable",
						getThemedContext().getPackageName()), false)) {
			...
			requestFeature(Window.FEATURE_NO_TITLE);
				...
				getThemedContext().getResources().getIdentifier(
						...
						getThemedContext().getPackageName()),
				// } else if
				// (a.getBoolean(R.styleable.SherlockTheme_windowActionBar,
				false)) {
		...
		if (a.getBoolean(
				getThemedContext().getResources().getIdentifier(
						"SherlockTheme_windowActionBarOverlay", "styleable",
						getThemedContext().getPackageName()),
				// if
				// (a.getBoolean(R.styleable.SherlockTheme_windowActionBarOverlay,
				false)) {
		...
		if (a.getBoolean(
				getThemedContext().getResources().getIdentifier(
						"SherlockTheme_windowActionModeOverlay", "styleable",
						getThemedContext().getPackageName()),
				// if
				// (a.getBoolean(R.styleable.SherlockTheme_windowActionModeOverlay,
				false)) {
		...
		if (!hasFeature(Window.FEATURE_NO_TITLE)) {
			if (hasFeature(Window.FEATURE_ACTION_BAR_OVERLAY)) {
				layoutResource = getThemedContext().getResources()
						.getIdentifier("abs__screen_action_bar_overlay",
								"layout", getThemedContext().getPackageName());
				// layoutResource = R.layout.abs__screen_action_bar_overlay;
			} else {
				layoutResource = getThemedContext().getResources()
						.getIdentifier("abs__screen_action_bar", "layout",
								getThemedContext().getPackageName());
				// layoutResource = R.layout.abs__screen_action_bar;
			}
		} else if (hasFeature(Window.FEATURE_ACTION_MODE_OVERLAY)
				&& !hasFeature(Window.FEATURE_NO_TITLE)) {
			layoutResource = getThemedContext().getResources().getIdentifier(
					"abs__screen_simple_overlay_action_mode", "layout",
					getThemedContext().getPackageName());
			// layoutResource = R.layout.abs__screen_simple_overlay_action_mode;
		} else {
			layoutResource = getThemedContext().getResources().getIdentifier(
					"abs__screen_simple", "layout",
					getThemedContext().getPackageName());
