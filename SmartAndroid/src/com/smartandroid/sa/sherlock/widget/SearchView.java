...
import static com.smartandroid.sa.sherlock.widget.SuggestionsAdapter.getColumnString;
 ...
 * href="{@docRoot}, read the <a href="{@docRoot} , read the <a href="{@docRoot}
 * , read the <a href="{@docRoot}, read the <a href="{@docRoot}, read the <a
 * href="{@docRoot} , read the <a href="{@docRoot} , read the <a
 * href="{@docRoot}, read the <a href="{@docRoot}, read the <a href="{@docRoot},
 * read the <a href="{@docRoot}, read the <a href="{@docRoot}, read the <a
 * href="{@docRoot}, read the <a href="{@docRoot} , read the <a href="{@docRoot}
 * , read the <a href="{@docRoot}, read the <a href="{@docRoot}, read the <a
...
public class SearchView extends LinearLayout implements CollapsibleActionView {
	...
	private Runnable mReleaseCursorRunnable = new Runnable() {
		public void run() {
			if (mSuggestionsAdapter != null
					&& mSuggestionsAdapter instanceof SuggestionsAdapter) {
	...
	public SearchView(Context context, AttributeSet attrs) {
		...
		inflater.inflate(
				getResources().getIdentifier("abs__search_view", "layout",
						context.getPackageName()), this, true);
		// inflater.inflate(R.layout.abs__search_view, this, true);

		mSearchButton = findViewById(getResources().getIdentifier(
				"abs__search_button", "id", context.getPackageName()));
		// mSearchButton = findViewById(R.id.abs__search_button);
		mQueryTextView = (SearchAutoComplete) findViewById(getResources()
				.getIdentifier("abs__search_src_text", "id",
						context.getPackageName()));
		// mQueryTextView = (SearchAutoComplete)
		// findViewById(R.id.abs__search_src_text);
		mQueryTextView.setSearchView(this);

		mSearchEditFrame = findViewById(getResources().getIdentifier(
				"abs__search_edit_frame", "id", context.getPackageName()));
		// mSearchEditFrame = findViewById(R.id.abs__search_edit_frame);
		mSearchPlate = findViewById(getResources().getIdentifier(
				"abs__search_plate", "id", context.getPackageName()));
		// mSearchPlate = findViewById(R.id.abs__search_plate);
		mSubmitArea = findViewById(getResources().getIdentifier(
				"abs__submit_area", "id", context.getPackageName()));
		// mSubmitArea = findViewById(R.id.abs__submit_area);
		mSubmitButton = findViewById(getResources().getIdentifier(
				"abs__search_go_btn", "id", context.getPackageName()));
		// mSubmitButton = findViewById(R.id.abs__search_go_btn);
		mCloseButton = (ImageView) findViewById(getResources().getIdentifier(
				"abs__search_close_btn", "id", context.getPackageName()));
		// mCloseButton = (ImageView) findViewById(R.id.abs__search_close_btn);
		mVoiceButton = findViewById(getResources().getIdentifier(
				"abs__search_voice_btn", "id", context.getPackageName()));
		// mVoiceButton = findViewById(R.id.abs__search_voice_btn);
		mSearchHintIcon = (ImageView) findViewById(getResources()
				.getIdentifier("abs__search_mag_icon", "id",
						context.getPackageName()));
		...
		TypedArray a = context.obtainStyledAttributes(
				attrs,
				new int[] { getResources().getIdentifier("SherlockSearchView",
						"styleable", context.getPackageName()) }, 0, 0);
		// R.styleable.SherlockSearchView, 0, 0);
		setIconifiedByDefault(a.getBoolean(
				getResources().getIdentifier(
						"SherlockSearchView_iconifiedByDefault", "styleable",
						context.getPackageName()), true));
		// R.styleable.SherlockSearchView_iconifiedByDefault, true));
		int maxWidth = a.getDimensionPixelSize(
				getResources().getIdentifier(
						"SherlockSearchView_android_maxWidth", "styleable",
						context.getPackageName()), -1);
		// R.styleable.SherlockSearchView_android_maxWidth, -1);
		if (maxWidth != -1) {
			setMaxWidth(maxWidth);
		}
		CharSequence queryHint = a.getText(getResources().getIdentifier(
				"SherlockSearchView_queryHint", "styleable",
				context.getPackageName()));
		...
		int imeOptions = a.getInt(
				getResources().getIdentifier(
						"SherlockSearchView_android_imeOptions", "styleable",
						context.getPackageName()), -1);
		...
		int inputType = a.getInt(
				getResources().getIdentifier(
						"SherlockSearchView_android_inputType", "styleable",
						context.getPackageName()), -1);
		...
		a = context.obtainStyledAttributes(
				attrs,
				new int[] { getResources().getIdentifier("SherlockView",
						"styleable", context.getPackageName()) }, 0,
				// a = context.obtainStyledAttributes(attrs,
				// R.styleable.SherlockView, 0,
				0);
		focusable = a.getBoolean(
				getResources().getIdentifier("SherlockView_android_focusable",
	...
	public boolean requestFocus(int direction, Rect previouslyFocusedRect) {
		...
		if (!isIconified()) {
			boolean result = mQueryTextView.requestFocus(direction,
	...
	public void setQueryRefinementEnabled(boolean enable) {
		...
		if (mSuggestionsAdapter instanceof SuggestionsAdapter) {
			((SuggestionsAdapter) mSuggestionsAdapter)
					.setQueryRefinement(enable ? SuggestionsAdapter.REFINE_ALL
	...
	private int getPreferredWidth() {
		return getContext().getResources().getDimensionPixelSize(
				getResources().getIdentifier(
						"abs__search_view_preferred_width", "dimen",
						c.getPackageName()));
		// R.dimen.abs__search_view_preferred_width);
	}
	...
	private void updateSubmitArea() {
		...
		if (isSubmitAreaEnabled()
				&& (mSubmitButton.getVisibility() == VISIBLE || mVoiceButton
	...
	private void updateCloseButton() {
		...
		final boolean showClose = hasText
				|| (mIconifiedByDefault && !mExpandedInActionView);
		mCloseButton.setVisibility(showClose ? VISIBLE : GONE);
		mCloseButton.getDrawable().setState(
				hasText ? ENABLED_STATE_SET : EMPTY_STATE_SET);
	}
	...
	private void updateFocusedState() {
		...
		mSearchPlate.getBackground().setState(
				focused ? FOCUSED_STATE_SET : EMPTY_STATE_SET);
		mSubmitArea.getBackground().setState(
				focused ? FOCUSED_STATE_SET : EMPTY_STATE_SET);
		invalidate();
	}
		...
		public boolean onKey(View v, int keyCode, KeyEvent event) {
			...
			if (DBG) {
				Log.d(LOG_TAG, "mTextListener.onKey(" + keyCode + "," + event
						+ "), selection: " + mQueryTextView.getListSelection());
			}
	...
	private boolean onSuggestionsKey(View v, int keyCode, KeyEvent event) {
		...
		if (event.getAction() == KeyEvent.ACTION_DOWN
			...
			if (keyCode == KeyEvent.KEYCODE_DPAD_LEFT
				...
				// Arabic
				int selPoint = (keyCode == KeyEvent.KEYCODE_DPAD_LEFT) ? 0
			...
			if (keyCode == KeyEvent.KEYCODE_DPAD_UP
					&& 0 == mQueryTextView.getListSelection()) {
	...
	private int getSearchIconId() {
		...
		getContext().getTheme().resolveAttribute(
				getResources().getIdentifier("searchViewSearchIcon", "attr",
						c.getPackageName()),
	...
	private CharSequence getDecoratedHint(CharSequence hintText) {
		...
		SpannableStringBuilder ssb = new SpannableStringBuilder("   "); // for
																		...
																		// icon
		ssb.append(hintText);
		Drawable searchIcon = getContext().getResources().getDrawable(
				getSearchIconId());
	...
	private void updateSearchAutoComplete() {
		...
		if (mSearchable.getSuggestAuthority() != null) {
			mSuggestionsAdapter = new SuggestionsAdapter(getContext(), this,
	...
	private void adjustDropDownSizeAndPosition() {
		if (mDropDownAnchor.getWidth() > 1) {
			...
			int iconOffset = mIconifiedByDefault ? res
					.getDimensionPixelSize(getResources().getIdentifier(
							"abs__dropdownitem_icon_width", "dimen",
							c.getPackageName()))
					// .getDimensionPixelSize(R.dimen.abs__dropdownitem_icon_width)
					+ res.getDimensionPixelSize(getResources().getIdentifier(
							"abs__dropdownitem_text_padding_left", "dimen",
							c.getPackageName()))
			// +
			...
			mQueryTextView
					.setDropDownHorizontalOffset(-(dropDownPadding.left + iconOffset)
							+ anchorPadding);
			mQueryTextView.setDropDownWidth(mDropDownAnchor.getWidth()
					+ dropDownPadding.left + dropDownPadding.right + iconOffset
					- (anchorPadding));
		}
	}
	 ...
	 *         position).
	 */
	private boolean launchSuggestion(int position, int actionKey,
			String actionMsg) {
		Cursor c = mSuggestionsAdapter.getCursor();
	...
	private void setQuery(CharSequence query) {
		...
		mQueryTextView.setSelection(TextUtils.isEmpty(query) ? 0 : query
				.length());
	}

	private void launchQuerySearch(int actionKey, String actionMsg, String query) {
		String action = Intent.ACTION_SEARCH;
		Intent intent = createIntent(action, null, null, query, actionKey,
				actionMsg);
		getContext().startActivity(intent);
	}
	 ...
	 * @return The intent.
	 */
	private Intent createIntent(String action, Uri data, String extraData,
			String query, int actionKey, String actionMsg) {
	 ...
	 * web search.
	 */
	private Intent createVoiceWebSearchIntent(Intent baseIntent,
			SearchableInfo searchable) {
		Intent voiceIntent = new Intent(baseIntent);
		ComponentName searchActivity = searchable.getSearchActivity();
		voiceIntent.putExtra(
				RecognizerIntent.EXTRA_CALLING_PACKAGE,
				searchActivity == null ? null : searchActivity
	...
	private Intent createVoiceAppSearchIntent(Intent baseIntent,
		...
		if (searchable.getVoiceLanguageModeId() != 0) {
			languageModel = resources.getString(searchable
	 ...
	 */
	private Intent createIntentFromSuggestion(Cursor c, int actionKey,
			String actionMsg) {
		try {
			// use specific action if supplied, or default action if supplied,
			// or fixed default
			String action = getColumnString(c,
			...
			String query = getColumnString(c,
					SearchManager.SUGGEST_COLUMN_QUERY);
			String extraData = getColumnString(c,
					SearchManager.SUGGEST_COLUMN_INTENT_EXTRA_DATA);

			return createIntent(action, dataUri, extraData, query, actionKey,
					actionMsg);
	...
	private void forceSuggestionQuery() {
		try {
			Method before = AutoCompleteTextView.class
					.getDeclaredMethod("doBeforeTextChanged");
			Method after = AutoCompleteTextView.class
	...
	private static void ensureImeVisible(AutoCompleteTextView view,
			boolean visible) {
		try {
			Method method = AutoCompleteTextView.class.getMethod(
					"ensureImeVisible", boolean.class);
	...
	private static void setText(AutoCompleteTextView view, CharSequence text,
			boolean filter) {
		try {
			Method method = AutoCompleteTextView.class.getMethod("setText",
