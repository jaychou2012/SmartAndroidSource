...
package com.smartandroid.sa.sherlock.view;
...
public interface MenuItem {
	/*
	 * These should be kept in sync with attrs.xml enum constants for
	 * showAsAction
	 */
	...
	/**
	 * Interface definition for a callback to be invoked when a menu item marked
	 * with {@link MenuItem#SHOW_AS_ACTION_COLLAPSE_ACTION_VIEW} is expanded or
	 * collapsed.
	 * 
	...
	public interface OnActionExpandListener {
		 ...
		 * {@link MenuItem#SHOW_AS_ACTION_COLLAPSE_ACTION_VIEW} is expanded.
		 * 
		 * @param item
		...
		/**
		 * Called when a menu item with
		 * {@link MenuItem#SHOW_AS_ACTION_COLLAPSE_ACTION_VIEW} is collapsed.
		 * 
		 * @param item
		 *            Item that was collapsed
		 * @return true if the item should collapse, false if collapsing should
		 *         be suppressed.
		 */
	...
	/**
	 * Return the identifier for this menu item. The identifier can not be
	 * changed after the menu is created.
	 * 
	...
	/**
	 * Return the category and order within the category of this item. This item
	 * will be shown before all items (within its category) that have order
	 * greater than this value.
	 * <p>
	 * An order integer contains the item's category (the upper bits of the
	 * integer; set by or/add the category with the order within the category)
	 * and the ordering of the item within that category (the lower bits).
	 * Example categories are {@link Menu#CATEGORY_SYSTEM},
	...
	/**
	 * Change the condensed title associated with this item. The condensed title
	 * is used in situations where the normal title may be too long to be
	 * displayed.
	 * 
	 * @param title
	...
	/**
	 * Retrieve the current condensed title of the item. If a condensed title
	 * was never set, it will return the normal title.
	 * 
	...
	/**
	 * Returns the icon for this item as a Drawable (getting it from resources
	 ...
	 * Intent associated with a menu item. If you set one, and nothing else
	 * handles the item, then the default behavior will be to call
	 * {@link android.content.Context#startActivity} with the given Intent.
	 * 
	 * <p>
	 * Note that setIntent() can not be used with the versions of
	 * {@link Menu#add} that take a Runnable, because {@link Runnable#run} does
	 * not return a value so there is no way to tell if it handled the item. In
	 * this case it is assumed that the Runnable always handles the item, and
	 * the intent will never be started.
	 * 
	 * @see #getIntent
	 * @param intent
	 *            The Intent to associated with the item. This Intent object is
	 *            <em>not</em> copied, so be careful not to modify it later.
	 * @return This Item so additional setters can be called.
	 */
	public MenuItem setIntent(Intent intent);

	/**
	 * Return the Intent associated with this item. This returns a reference to
	 * the Intent which you can change as desired to modify what the Item is
	 * holding.
	 * 
	 * @see #setIntent
	 ...
	 * actually display a check mark (see {@link #setChecked} for that); rather,
	 ...
	 * 
	 * @param checkable
	 *            Set to true to allow a check mark, false to disallow. The
	 *            default is false.
	 * @see #setChecked
	 * @see #isCheckable
	...
	/**
	 * Control whether this item is shown with a check mark. Note that you must
	 * first have enabled checking with {@link #setCheckable} or else the check
	 * mark will not appear. If this item is a member of a group that contains
	 * mutually-exclusive items (set via
	 * {@link Menu#setGroupCheckable(int, boolean, boolean)}, the other items in
	 ...
	 * @see Menu#setGroupCheckable
	 * @param checked
	 *            Set to true to display a check mark, false to hide it. The
	 ...
	 * 
	 * @param enabled
	 *            If true then the item will be invokable; if false it is won't
	 *            be invokable.
	 * @return This Item so additional setters can be called.
	 */
	...
	/**
	 * Check whether this item has an associated sub-menu. I.e. it is a sub-menu
	 * of another menu.
	 * 
	 ...
	 * information is set by the View that added this menu item to the menu.
	 * 
	 * @see OnCreateContextMenuListener
	 * @return The extra information linked to the View that added this menu
	 *         item to the menu. This can be null.
	 */
	 ...
	 * {@link #SHOW_AS_ACTION_WITH_TEXT}. SHOW_AS_ACTION_WITH_TEXT requests that
	 * when the item is shown as an action, it should be shown with a text
	 * label.
	 * 
	 * @param actionEnum
	 ...
	 *            {@link #SHOW_AS_ACTION_NEVER}. SHOW_AS_ACTION_NEVER is the
	 *            default.
	 * 
	 ...
	 * 
	 * <p>
	 * Note: This method differs from {@link #setShowAsAction(int)} only in that
	 * it returns the current MenuItem instance for call chaining.
	 * 
	 * @param actionEnum
	 *            How the item should display. One of
	 *            {@link #SHOW_AS_ACTION_ALWAYS},
	 ...
	 * <p>
	 * <strong>Note:</strong> Setting an action view overrides the action
	 * provider set via {@link #setActionProvider(ActionProvider)}.
	 * </p>
	 * 
	 * @param view
	...
	/**
	 * Sets the {@link ActionProvider} responsible for creating an action view
	 * if the item is placed on the action bar. The provider also provides a
	 * default action invoked if the item is placed in the overflow menu.
	 * <p>
	 * <strong>Note:</strong> Setting an action provider overrides the action
	 * view set via {@link #setActionView(int)} or {@link #setActionView(View)}.
	 * </p>
	 * 
	...
	/**
	 * Expand the action view associated with this menu item. The menu item must
	 * have an action view set, as well as the showAsAction flag
	 * {@link #SHOW_AS_ACTION_COLLAPSE_ACTION_VIEW}. If a listener has been set
	 * using {@link #setOnActionExpandListener(OnActionExpandListener)} it will
	 * have its {@link OnActionExpandListener#onMenuItemActionExpand(MenuItem)}
	 * method invoked. The listener may return false from this method to prevent
	...
	/**
	 * Collapse the action view associated with this menu item. The menu item
	 * must have an action view set, as well as the showAsAction flag
	 * {@link #SHOW_AS_ACTION_COLLAPSE_ACTION_VIEW}. If a listener has been set
	 * using {@link #setOnActionExpandListener(OnActionExpandListener)} it will
	 * have its
	 * {@link OnActionExpandListener#onMenuItemActionCollapse(MenuItem)} method
	 * invoked. The listener may return false from this method to prevent
	...
	/**
	 * Set an {@link OnActionExpandListener} on this menu item to be notified
	 * when the associated action view is expanded or collapsed. The menu item
	 * must be configured to expand or collapse its action view using the flag
