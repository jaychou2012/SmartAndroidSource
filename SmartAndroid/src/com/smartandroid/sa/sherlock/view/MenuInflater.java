	...
	private class MenuState {
		...
		public void readGroup(AttributeSet attrs) {
			TypedArray a = mContext.obtainStyledAttributes(
					attrs,
					new int[] { mContext.getResources().getIdentifier(
							"SherlockMenuGroup", "styleable",
							mContext.getPackageName()) });
			// R.styleable.SherlockMenuGroup);

			groupId = a.getResourceId(
					mContext.getResources().getIdentifier(
							"SherlockMenuGroup_android_id", "styleable",
							mContext.getPackageName()),
					// groupId =
					// a.getResourceId(R.styleable.SherlockMenuGroup_android_id,
					defaultGroupId);
			groupCategory = a.getInt(
					mContext.getResources().getIdentifier(
							"SherlockMenuGroup_android_menuCategory",
							"styleable", mContext.getPackageName()),
			...
			groupVisible = a.getBoolean(
					mContext.getResources().getIdentifier(
							"SherlockMenuGroup_android_visible", "styleable",
							mContext.getPackageName()),
					// R.styleable.SherlockMenuGroup_android_visible,
					defaultItemVisible);
			groupEnabled = a.getBoolean(
					mContext.getResources().getIdentifier(
							"SherlockMenuGroup_android_enabled", "styleable",
							mContext.getPackageName()),
		...
		public void readItem(AttributeSet attrs) {
			TypedArray a = mContext.obtainStyledAttributes(
					attrs,
					new int[] { mContext.getResources().getIdentifier(
							"SherlockMenuItem", "styleable",
							mContext.getPackageName()) });
			// R.styleable.SherlockMenuItem);

			// Inherit attributes from the group as default value
			itemId = a.getResourceId(
					mContext.getResources().getIdentifier(
							"SherlockMenuItem_android_id", "styleable",
							mContext.getPackageName()),
					// itemId =
					// a.getResourceId(R.styleable.SherlockMenuItem_android_id,
					defaultItemId);
			...
			itemTitle = a.getText(mContext.getResources().getIdentifier(
					"SherlockMenuItem_android_title", "styleable",
					mContext.getPackageName()));
			// itemTitle =
			// a.getText(R.styleable.SherlockMenuItem_android_title);
			itemTitleCondensed = a.getText(mContext.getResources()
					.getIdentifier("SherlockMenuItem_android_titleCondensed",
							"styleable", mContext.getPackageName()));
			// .getText(R.styleable.SherlockMenuItem_android_titleCondensed);
			itemIconResId = a.getResourceId(
					mContext.getResources().getIdentifier(
							"SherlockMenuItem_android_icon", "styleable",
							mContext.getPackageName()), 0);
			// R.styleable.SherlockMenuItem_android_icon, 0);
			itemAlphabeticShortcut = getShortcut(a.getString(mContext
					.getResources().getIdentifier(
							"SherlockMenuItem_android_alphabeticShortcut",
							"styleable", mContext.getPackageName())));
			// .getString(R.styleable.SherlockMenuItem_android_alphabeticShortcut));
			itemNumericShortcut = getShortcut(a.getString(mContext
					.getResources().getIdentifier(
							"SherlockMenuItem_android_numericShortcut",
							"styleable", mContext.getPackageName())));
			// .getString(R.styleable.SherlockMenuItem_android_numericShortcut));
			if (a.hasValue(mContext.getResources().getIdentifier(
					"SherlockMenuItem_android_checkable", "styleable",
					mContext.getPackageName()))) {
				// if
				// (a.hasValue(R.styleable.SherlockMenuItem_android_checkable))
				// {
				// Item has attribute checkable, use it
				itemCheckable = a.getBoolean(
						mContext.getResources().getIdentifier(
								"SherlockMenuItem_android_checkable",
								"styleable", mContext.getPackageName()), false) ? 1
			...
			itemChecked = a.getBoolean(
					mContext.getResources().getIdentifier(
							"SherlockMenuItem_android_checked", "styleable",
							mContext.getPackageName()),
					// R.styleable.SherlockMenuItem_android_checked,
					defaultItemChecked);
			itemVisible = a.getBoolean(
					mContext.getResources().getIdentifier(
							"SherlockMenuItem_android_visible", "styleable",
							mContext.getPackageName()), groupVisible);
			// R.styleable.SherlockMenuItem_android_visible, groupVisible);
			itemEnabled = a.getBoolean(
					mContext.getResources().getIdentifier(
							"SherlockMenuItem_android_enabled", "styleable",
							mContext.getPackageName()), groupEnabled);
			...
			a.getValue(
					mContext.getResources().getIdentifier(
							...
							"styleable", mContext.getPackageName()), value);
			// a.getValue(R.styleable.SherlockMenuItem_android_showAsAction,
			// value);
			itemShowAsAction = value.type == TypedValue.TYPE_INT_HEX ? value.data
					: -1;

			itemListenerMethodName = a.getString(mContext.getResources()
					.getIdentifier("SherlockMenuItem_android_onClick",
							"styleable", mContext.getPackageName()));
			...
			itemActionViewLayout = a.getResourceId(
					mContext.getResources().getIdentifier(
							...
							"styleable", mContext.getPackageName()), 0);
