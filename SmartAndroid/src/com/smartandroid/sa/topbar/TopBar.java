...
public class TopBar extends RelativeLayout implements OnClickListener {
	...
	public TopBar(Context context, AttributeSet attrs) {
		...
		mBarView = (RelativeLayout) mInflater.inflate(getResources()
				.getIdentifier("topbar", "layout", context.getPackageName()),
				null);

		addView(mBarView);

		mLogoView = (ImageView) mBarView.findViewById(getResources()
				.getIdentifier("topbar_home_logo", "id",
						context.getPackageName()));

		mHomeLayout = (RelativeLayout) mBarView
				.findViewById(getResources().getIdentifier("topbar_home_bg",
						"id", context.getPackageName()));

		mHomeBtn = (ImageButton) mBarView.findViewById(getResources()
				.getIdentifier("topbar_home_btn", "id",
						context.getPackageName()));

		mBackIndicator = mBarView.findViewById(getResources().getIdentifier(
				"topbar_home_is_back", "id", context.getPackageName()));

		mTitleView = (TextView) mBarView.findViewById(getResources()
				.getIdentifier("topbar_title", "id", context.getPackageName()));

		mActionsView = (LinearLayout) mBarView
				.findViewById(getResources().getIdentifier("topbar_actions",
						"id", context.getPackageName()));

		mProgress = (ProgressBar) mBarView.findViewById(getResources()
				.getIdentifier("topbar_progress", "id",
						context.getPackageName()));

		TypedArray a = context.obtainStyledAttributes(
				attrs,
				new int[] { getResources().getIdentifier("TopBar", "styleable",
						context.getPackageName()) });
		// TypedArray a = context.obtainStyledAttributes(attrs,
		// R.styleable.ActionBar);
		CharSequence title = a.getString(getResources().getIdentifier(
				"TopBar_title", "styleable", context.getPackageName()));
	...
	private View inflateAction(Action action) {
		View view = mInflater.inflate(
				getResources().getIdentifier("topbar_item", "layout",
						c.getPackageName()), mActionsView, false);

		ImageButton labelView = (ImageButton) view
				.findViewById(getResources().getIdentifier("topbar_item", "id",
						c.getPackageName()));
//		ImageButton labelView = (ImageButton) view
