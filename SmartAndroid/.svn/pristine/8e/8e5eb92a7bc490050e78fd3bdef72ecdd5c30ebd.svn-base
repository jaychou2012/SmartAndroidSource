...
import com.tandong.sa.json.internal.$Gson$Types;
 ...
 * @author Joel Leitch
 */
final class JsonArrayDeserializationVisitor<T> extends
		JsonDeserializationVisitor<T> {
	...
	JsonArrayDeserializationVisitor(JsonArray jsonArray, Type arrayType,
		...
		super(jsonArray, arrayType, objectNavigator, fieldNamingPolicy,
	...
	protected T constructTarget() {
		...
		if ($Gson$Types.isArray(targetType)) {
			// We know that we are getting back an array of the required type,
			// so
			// this typecasting is safe.
			return (T) objectConstructor.constructArray(
					$Gson$Types.getArrayComponentType(targetType),
					jsonArray.size());
		}
		// is a collection
		return (T) objectConstructor.construct($Gson$Types
				.getRawType(targetType));
	}
	...
	public void visitArray(Object array, Type arrayType) {
		...
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			if (jsonChild == null || jsonChild.isJsonNull()) {
				...
				child = visitChildAsObject(
						$Gson$Types.getArrayComponentType(arrayType), jsonChild);
			} else if (jsonChild instanceof JsonArray) {
				child = visitChildAsArray(
						$Gson$Types.getArrayComponentType(arrayType),
	...
	public void startVisitingObject(Object node) {
		throw new JsonParseException("Expecting array but found object: "
				+ node);
	}

	public void visitArrayField(FieldAttributes f, Type typeOfF, Object obj) {
		throw new JsonParseException("Expecting array but found array field "
				+ f.getName() + ": " + obj);
	}

	public void visitObjectField(FieldAttributes f, Type typeOfF, Object obj) {
		throw new JsonParseException("Expecting array but found object field "
				+ f.getName() + ": " + obj);
	}

	public boolean visitFieldUsingCustomHandler(FieldAttributes f,
			Type actualTypeOfField, Object parent) {
		throw new JsonParseException("Expecting array but found field "
				+ f.getName() + ": " + parent);
	}
