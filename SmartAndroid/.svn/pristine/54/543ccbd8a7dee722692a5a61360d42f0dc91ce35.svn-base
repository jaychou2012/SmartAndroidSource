...
public class CollisionHelper {
	...
	// we don't want these to be instatianted for every call to
	// calculateLineSpaceForGivenYOffset, we'll just reuse them
	private final static ArrayList<Area> mAreas = new ArrayList<Area>();

	public static Line calculateLineSpaceForGivenYOffset(float lineYbottom,
			int lineHeight, float viewWidth, ArrayList<Obstacle> obstacles) {
		...
		for (Obstacle obstacle : obstacles) {
			...
			if (obstacle.topLefty > lineYbottom
					|| obstacle.bottomRighty < lineYtop) {
				...
				for (Obstacle innerObstacle : obstacles) {
					if (innerObstacle.topLefty > lineYbottom
							|| innerObstacle.bottomRighty < lineYtop) {
