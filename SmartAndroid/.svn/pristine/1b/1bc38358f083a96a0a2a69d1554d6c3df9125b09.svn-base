...
public abstract class FadingActionBarHelperBase {
	...
	private View createWebView() {
		ViewGroup webViewContainer = (ViewGroup) mInflater.inflate(
				c.getResources().getIdentifier("fab__webview_container",
						"layout", c.getPackageName()), null);
		...
		mHeaderContainer = (FrameLayout) webViewContainer.findViewById(c
				.getResources().getIdentifier("fab__header_container", "id",
						c.getPackageName()));
	...
	private View createScrollView() {
		ViewGroup scrollViewContainer = (ViewGroup) mInflater.inflate(
				c.getResources().getIdentifier("fab__scrollview_container",
						"layout", c.getPackageName()), null);
		// R.layout.fab__scrollview_container, null);

		ObservableScrollView scrollView = (ObservableScrollView) scrollViewContainer
				.findViewById(c.getResources().getIdentifier(
						"fab__scroll_view", "id", c.getPackageName()));
		...
		ViewGroup contentContainer = (ViewGroup) scrollViewContainer
				.findViewById(c.getResources().getIdentifier("fab__container",
						"id", c.getPackageName()));
		...
		mHeaderContainer = (FrameLayout) scrollViewContainer.findViewById(c
				.getResources().getIdentifier("fab__header_container", "id",
						c.getPackageName()));
		// .findViewById(R.id.fab__header_container);
		initializeGradient(mHeaderContainer);
		mHeaderContainer.addView(mHeaderView, 0);
		mMarginView = (FrameLayout) contentContainer.findViewById(c
				.getResources().getIdentifier("fab__content_top_margin", "id",
						c.getPackageName()));
	...
	private View createListView(ListView listView) {
		ViewGroup contentContainer = (ViewGroup) mInflater.inflate(
				c.getResources().getIdentifier("fab__listview_container",
						"layout", c.getPackageName()), null);
		// R.layout.fab__listview_container, null);
		contentContainer.addView(mContentView);

		mHeaderContainer = (FrameLayout) contentContainer.findViewById(c
				.getResources().getIdentifier("fab__header_container", "id",
						c.getPackageName()));
		// .findViewById(R.id.fab__header_container);
		initializeGradient(mHeaderContainer);
		...
		mListViewBackgroundView = contentContainer.findViewById(c
				.getResources().getIdentifier("fab__listview_background", "id",
						c.getPackageName()));
	...
	private void initializeGradient(ViewGroup headerContainer) {
		View gradientView = headerContainer.findViewById(c.getResources()
				.getIdentifier("fab__gradient", "id", c.getPackageName()));
		// View gradientView = headerContainer.findViewById(R.id.fab__gradient);
		int gradient = c.getResources().getIdentifier("fab__gradient",
				"drawable", c.getPackageName());
		// int gradient = R.drawable.fab__gradient;
		if (mLightActionBar) {
			gradient = c.getResources().getIdentifier("fab__gradient_light",
					"drawable", c.getPackageName());
