...
public class SpanParser {
	...
	public float parseSpans(ArrayList<HtmlObject> objects, Object[] spans,
	...
	private HtmlObject getHtmlObject(String content, int start, int end,
			float thisXOffset) {
		return new HtmlObject(content, start, end, thisXOffset,
				mFlowTextView.getTextPaint());
	}
	...
	private HtmlLink getHtmlLink(URLSpan span, String content, int start,
		...
		HtmlLink obj = new HtmlLink(content, start, end, thisXOffset,
				mFlowTextView.getLinkPaint(), span.getURL());
		mLinks.add(obj);
		return obj;
	}

	public void addLink(HtmlLink thisLink, float yOffset, float width,
			float height) {
